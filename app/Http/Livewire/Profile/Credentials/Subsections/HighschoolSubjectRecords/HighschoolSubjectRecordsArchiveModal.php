...
class HighschoolSubjectRecordsArchiveModal extends Component
{
    use WithPagination;
    use SelectRows;
    use Toastable;
    ...
    public string $modelClass = HighschoolSubjectRecord::class;
    public int $perPage = 15;

    protected $listeners = [
        'openHighschoolSubjectArchive' => 'open',
        'refreshHighschoolSubjectArchive' => 'handleRefresh',
    ];
    ...
    {
        $this->emit('confirmHighschoolSubjectRestore', [
            'ids' => [$id],
    ...
    public function prepareBulkAction(string $action)
    {
        if ($action === 'restoreSelected') {
            ...
            $this->emit('confirmHighschoolSubjectForceDelete', [
                'ids' => $this->selected,
                'modelClass' => $this->modelClass,
            ]);
        }
    ...
    public function forceDeleteSelected()
    {
        ...
        $this->dispatch('savedCredential');
        $labels = $models->map(function ($m) {
            return optional($m->subject)->name ?? $m->name ?? (string) ($m->id ?? '');
        })->values()->all();
        $count = count($labels);
        if ($count === 1) {
    ...
    public function render()
    {
        ...
        return view('livewire.profile.credentials.subsections.highschool_subject_records.archive-modal', compact('items'));
    }
