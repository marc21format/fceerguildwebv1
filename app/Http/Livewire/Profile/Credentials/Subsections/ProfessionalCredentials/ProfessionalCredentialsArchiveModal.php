...
class ProfessionalCredentialsArchiveModal extends Component
{
    use WithPagination;
    use SelectRows;
    use Toastable;
    ...
    public string $modelClass = ProfessionalCredential::class;
    public int $perPage = 15;

    protected $listeners = [
        'openProfessionalArchive' => 'open',
        'refreshProfessionalArchive' => 'handleRefresh',
    ];
    ...
    {
        $this->emit('confirmProfessionalRestore', [
            'ids' => [$id],
    ...
    public function prepareBulkAction(string $action)
    {
        if ($action === 'restoreSelected') {
            ...
            $this->emit('confirmProfessionalForceDelete', [
                'ids' => $this->selected,
                'modelClass' => $this->modelClass,
            ]);
        }
    ...
    public function render()
    {
        ...
        return view('livewire.profile.credentials.subsections.professional_credentials.archive-modal', compact('items'));
    }
