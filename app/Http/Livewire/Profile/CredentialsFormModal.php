...
class CredentialsFormModal extends Component
{
    ...
    // listen for the credentials-specific open event only
    protected $listeners = ['openCredentialsModal' => 'openCredentialsModal'];

    protected $rules = [];

    public function openCredentialsModal($params = [])
    {
        ...
        // If caller provided fields, use them; otherwise load from profile config for credentials
        if (!empty($params['fields']) && is_array($params['fields'])) {
            $this->fields = $params['fields'];
        } else {
            $cfg = config('profile.credentials');
        ...
        // Resolve select options from reference-tables or arrays
        $this->options = [];
        foreach ($this->fields as $field) {
    ...
    public function save()
    {
        ...
        // Note: address nested creation/updating is intentionally handled by the section component or model actions.
    ...
    public function render()
    {
        ...
        return view('livewire.profile.credentials.modals.form');
    }
}
