...
class ClassroomResponsibilitiesConfirmModal extends Component
{
    use Toastable;
    ...
    protected $listeners = [
        'confirmClassroomResponsibilitySave' => 'confirmPayload',
    ];

    public function confirmPayload($payload)
    {
        $modelClass = $payload['modelClass'] ?? null;
        if ($modelClass !== \App\Models\ClassroomResponsibility::class && ($payload['instanceKey'] ?? null) !== \App\Models\ClassroomResponsibility::class) {
            return;
        }
    ...
    public function confirm()
    {
        ...
        try {
            DB::transaction(function () use (&$m, $logger) {
                $modelClass = $this->modelClass ?? \App\Models\ClassroomResponsibility::class;
                if ($this->id) {
            ...
            if (! $label) {
                $name = optional($m->classroom)->name ?? ($m->classroom_id ? 'Classroom #' . $m->classroom_id : null);
                $pos = optional($m->classroomPosition)->name ?? ($m->classroom_position_id ? 'Position #' . $m->classroom_position_id : null);
    ...
    {
        return view('livewire.profile.fceer.subsections.classroom_responsibilities.confirm-changes-modal', [
