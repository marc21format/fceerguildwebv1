...
class CommitteeMembershipsArchive extends Component
{
    ...
    public function forceDeleteSelected()
    {
        ...
        $this->clearSelection();
        $labels = $models->map(function ($m) {
            return optional($m->committee)->name ?? (string) ($m->committee_id ?? '');
        })->values()->all();
        $count = count($labels);
        if ($count === 1) {
    ...
    {
        $items = ($this->modelClass)::onlyTrashed()->with('deletedBy','committee','committeePosition')->paginate($this->perPage);
