...
class Students extends Component
{
    ...
    public function prepareConfirmSave($userId, $fullName)
    {
        ...
        try { $this->dispatchBrowserEvent('open-confirm-modal'); } catch (\Throwable $_) { }
    }
    ...
    public function render()
    {
        ...
        if ($this->monthlyView && $reviewSeason && $totalStudents > 0) {
            ...
            while ($current->lte($seasonEnd)) {
                ...
                if (count($monthWeekends) > 0) {
                    $monthRecordsAll = Attendance::whereIn('user_id', $studentUserIds)
                        ->whereIn('date', $monthWeekends)
                        ->get();

                    $monthStatsAll = $this->calculateStudentAnalytics($monthRecordsAll, $monthWeekends, $studentUserIds);
                    $monthStatsAll['label'] = $current->format('F Y');
                    $monthlyAnalyticsCombined[] = $monthStatsAll;
                }
        ...
        if (!$this->monthlyView && $totalStudents > 0 && $reviewSeason) {
            ...
            for ($i = 0; $i < count($allWeekends); $i++) {
                ...
                if ($saturday || $sunday) {
                    ...
                    // Also compute combined (AM+PM) week analytics
                    $weekRecordsAll = Attendance::whereIn('user_id', $studentUserIds)
                        ->whereIn('date', $weekDates)
                        ->get();
                    $weekDataAll = $this->calculateStudentAnalytics($weekRecordsAll, $weekDates, $studentUserIds);
                    $weekDataAll['week_number'] = $weekNumber;
                    $weekDataAll['dates'] = $weekDates;
                    $weeklyAnalyticsCombined['weeks'][] = $weekDataAll;
        ...
        return view('livewire.attendance.students', [
            ...
            'monthlyAnalyticsCombined' => $monthlyAnalyticsCombined,
            'weeklyAnalytics' => $weeklyAnalytics,
            'weeklyAnalyticsCombined' => $weeklyAnalyticsCombined ?? ['weeks' => []],
        ]);
    }
    ...
    {
        // We'll count attendance per date *per session* (AM and PM) so totals reflect session counts.
        $result = ['on_time' => 0, 'late' => 0, 'absent' => 0, 'excused' => 0, 'present' => 0, 'total_days' => count($dates)];
        $totalStudents = count($studentUserIds);

        // Group records by date and session for fast lookup
        $recordsByDateSession = [];
        foreach ($records as $r) {
            $dKey = \Carbon\Carbon::parse($r->date)->format('Y-m-d');
            $sKey = strtolower($r->session ?? 'am');
            $recordsByDateSession[$dKey][$sKey][] = $r;
        }

        foreach ($dates as $date) {
            // For each weekend date, consider both AM and PM sessions
            foreach (['am', 'pm'] as $sess) {
                $daySessionRecords = collect($recordsByDateSession[$date][$sess] ?? []);
                $studentsWithValidStatus = [];

                foreach ($daySessionRecords as $rec) {
                    $status = strtolower($rec->student_status ?? '');
                    ...
                    if ($status === 'excused' || $rec->letter_id) {
                        ...
                        if ($status === 'late') {
                            ...
                            $timeIn = \Carbon\Carbon::parse($rec->attendance_time);
                            $sessionLower = strtolower($rec->session ?? $sess);
                            $isLate = ($sessionLower === 'am' && $timeIn->format('H:i') > '08:15') ||
                                      ($sessionLower === 'pm' && $timeIn->format('H:i') > '12:15');
                            if ($isLate) {
                                $result['late']++;
                ...
                // Students with no record for this date+session are absent for that session
