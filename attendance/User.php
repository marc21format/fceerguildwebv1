...
class User extends Component
{
    ...
    // Excuse letter modal
    public $showExcuseModal = false;
    public $reason;
    public $letter_file;
    public $date_attendance;
    public $session_choice = 'am';

    public function mount($userId)
    {
        // Restrict instructors (role_id 3) and students (role_id 4)
        // so they may only view their own attendance page.
        $actor = Auth::user();
        if ($actor && in_array($actor->role_id, [3, 4]) && (int) $actor->id !== (int) $userId) {
            abort(403, 'You are not authorized to view other users\' attendance.');
        }
    ...
    public function openExcuseModal($attendanceId, $session)
    {
        $this->showExcuseModal = true;
        $this->date_attendance = $this->selectedDate ?? now()->format('Y-m-d');
        $this->session_choice = $session;
    }

    public function closeExcuseModal()
    {
        $this->showExcuseModal = false;
        $this->reset(['reason', 'letter_file', 'date_attendance', 'session_choice']);
    }

    public function saveExcuseLetter()
    {
        $this->validate([
        ...
        \App\Models\StudentExcuseLetter::create([
            'user_id' => $this->user->id,
            'reason' => $this->reason,
            // database column is `letter_link` - save path there
            'letter_link' => $filePath,
            'date_attendance' => $this->date_attendance,
            'session_choice' => $this->session_choice,
            'status' => 'pending',
        ]);

        $this->closeExcuseModal();
        session()->flash('message', 'Excuse letter submitted successfully.');
        $this->loadAttendanceRecords(); // Refresh records
    }
