...
<div class="attendance-page" style="font-family: 'Inter', 'Segoe UI', system-ui, sans-serif;">
    <div class="attendance-wrapper">
        ...
        <div class="rounded-xl border border-slate-200 bg-card shadow-sm overflow-hidden">
            ...
            <div class="px-5 pt-5 pb-3">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @if(!empty($monthlyView) && $monthlyView)
                            <button type="button" class="text-sm text-slate-500 hover:text-slate-700 transition flex items-center gap-1" wire:click.prevent="toggleMonthlyView">
            ...
            {{-- Filters --}}
            <div class="px-5 pb-4">
                <form wire:submit.prevent="applyFilters" class="flex items-end gap-3 rounded-lg border border-slate-200 bg-card p-3">
                    <div class="flex flex-col gap-1 flex-1">
                        <label class="text-[10px] uppercase tracking-widest text-slate-500">Group</label>
                        <select wire:model.defer="committeeFilter" class="w-full appearance-none rounded-md border border-slate-200 bg-card px-3 py-2 text-sm text-slate-600 focus:border-slate-400 focus:outline-none" style="background-image: url(&quot;data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e&quot;); background-position: right 0.5rem center; background-repeat: no-repeat; background-size: 1.5em 1.5em; padding-right: 2.5rem;">
                ...
                @elseif($reviewSeason)
                    <div class="flex items-center gap-3 mt-3 p-3 rounded-lg border border-slate-200 bg-card">
                            ...
                            <div class="rounded-lg border border-slate-200 bg-card p-4">
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <div class="flex gap-1">
                                        <button type="button" class="text-slate-400 hover:text-slate-600 px-2 py-1 text-sm" wire:click.prevent="prevCalendarMonth" aria-label="Previous month"><i class="fa fa-chevron-left"></i></button>
                                        <button type="button" class="text-slate-400 hover:text-slate-600 px-2 py-1 text-sm" wire:click.prevent="nextCalendarMonth" aria-label="Next month"><i class="fa fa-chevron-right"></i></button>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-7 gap-1 text-center">
                                    ...
                                    @for($i = 1; $i <= $daysInMonth; $i++)
                                        ...
                                        @endphp
                                        <div
                                            class="h-8 flex items-center justify-center text-sm rounded cursor-pointer transition {{ $isToday ? 'bg-slate-700 text-white font-semibold' : 'text-slate-600 hover:bg-card' }}"
                ...
                <div class="px-5 pb-5">
                    <div class="rounded-lg border border-slate-200 bg-card p-4">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="flex items-center gap-2">
                                <button wire:click="prevWeeklyAnalyticsMonth" 
                                    class="px-2 py-1 text-xs rounded border border-slate-300 hover:bg-card transition-colors">
                                    â† Prev
                                </button>
                        ...
                        @php
                            ...
                            // Aggregate weeklyAnalytics entries (combine AM and PM sessions) so chart is session-independent
                            $weekAgg = [];
                            $wa = $weeklyAnalyticsCombined['weeks'] ?? ($weeklyAnalytics['weeks'] ?? []);
                            foreach ($wa as $wData) {
                                $weekNum = $wData['week_number'] ?? 1;
                                $days = $wData['total_days'] ?? 1;
                                if (!isset($weekAgg[$weekNum])) {
                                    $weekAgg[$weekNum] = [
                                        'on_time' => 0,
                                        'late' => 0,
                                        'absent' => 0,
                                ...
                                $weekAgg[$weekNum]['on_time'] += $wData['on_time'] ?? 0;
                                $weekAgg[$weekNum]['late'] += $wData['late'] ?? 0;
                                $weekAgg[$weekNum]['absent'] += $wData['absent'] ?? 0;
                                $weekAgg[$weekNum]['excused'] += $wData['excused'] ?? 0;
                                $weekAgg[$weekNum]['days'] = max($weekAgg[$weekNum]['days'], $days);
                            }

                            ksort($weekAgg);
                            foreach ($weekAgg as $weekNum => $vals) {
                                $weekLabels[] = 'Week ' . $weekNum;
                                // average across days and across AM+PM sessions
                                $denom = max(1, $vals['days'] * 2);
                                $weekOnTime[] = (($vals['on_time'] ?? 0) / $denom);
                                $weekLate[] = (($vals['late'] ?? 0) / $denom);
                                $weekAbsent[] = (($vals['absent'] ?? 0) / $denom);
                                $weekExcused[] = (($vals['excused'] ?? 0) / $denom);
                            }
                        ...
                        <div wire:ignore
                            wire:key="student-weekly-chart-{{ $weeklyAnalytics['year'] ?? '' }}-{{ $weeklyAnalytics['month'] ?? '' }}"
                            x-data="{
                                weeklyChart: null,
                                ...
                                init() {
                                    this.$nextTick(() => {
                                        ...
                                        const ctx = this.$refs.weeklyCanvas;
                                            if (ctx && window.Chart) {
                                            const totalStudents = this.chartData.total || 0;
                                            this.weeklyChart = new Chart(ctx, {
                                                ...
                                                options: {
                                                    ...
                                                    scales: {
                                                        ...
                                                        y: { stacked: true, beginAtZero: true, title: { display: true, text: 'Average Students per Day', font: { size: 11 } }, ticks: { callback: function(v) { return Number(v).toFixed(1); } } }
                                                    },
                                                    plugins: {
                                                        legend: { position: 'bottom', labels: { boxWidth: 12, padding: 15, font: { size: 10 } } },
                                                        tooltip: { callbacks: { label: function(ctx) { var val = Number(ctx.raw); var pct = totalStudents ? ((val / totalStudents) * 100).toFixed(1) : '0.0'; return ctx.dataset.label + ': ' + val.toFixed(1) + ' (' + pct + '%) — ' + val.toFixed(1) + '/' + totalStudents; } } }
                                                    }
                                                }
                                            });
                                        }
                ...
                <div class="px-5 pb-5">
                    <div class="rounded-lg border border-slate-200 bg-card p-4">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <div class="flex gap-1">
                                <button type="button" class="text-slate-400 hover:text-slate-600 px-2 py-1 text-sm" wire:click.prevent="prevMatrixMonth" aria-label="Previous month"><i class="fa fa-chevron-left"></i></button>
                                <button type="button" class="text-slate-400 hover:text-slate-600 px-2 py-1 text-sm" wire:click.prevent="nextMatrixMonth" aria-label="Next month"><i class="fa fa-chevron-right"></i></button>
                            </div>
                        </div>
                                        ...
                                        <tr class="hover:bg-card transition {{ $isEditing ? 'bg-amber-50' : '' }}">
                                            ...
                                            @foreach($weekendDates as $wd)
                                                ...
                                                <td class="py-3 px-2 text-center">
                                                    @php
                                                        // Determine status value for select preselection
                                                        $statusValue = 'absent';
                                                        if (!$isWithinSeason) {
                                                            $statusValue = 'n/a';
                                                        } elseif (!$ar) {
                                                            ...
                                                            if (strtolower($ar->student_status ?? '') === 'excused') {
                                                                $statusValue = 'excused';
                                                            } elseif (strtolower($ar->student_status ?? '') === 'late') {
                                                                $statusValue = 'late';
                                                            } elseif (strtolower($ar->student_status ?? '') === 'on time') {
                                                                $statusValue = 'on time';
                                                            } elseif ($ar->attendance_time) {
                                                                ...
                                                                $sessionLower = strtolower($ar->session ?? 'am');
                                                                $isLate = false;
                                                                if ($sessionLower === 'am' && $timeIn->format('H:i') > '08:15') $isLate = true;
                                                                if ($sessionLower === 'pm' && $timeIn->format('H:i') > '12:15') $isLate = true;
                                                                $statusValue = $isLate ? 'late' : 'on time';
                                                            }
                                                        }
                                                    @endphp
                                                    ...
                                                    @if($isEditing && in_array(auth()->user()->role_id ?? 0, [1, 2]) && $isWithinSeason)
                                                        @php
                                                            $statusClassSelect = match($statusValue) {
                                                        ...
                                                        <div class="flex flex-col items-center gap-1">
                                                            ...
                                                            <select 
                                                                class="appearance-none rounded-full px-2.5 py-0.5 text-[10px] font-semibold border-0 cursor-pointer focus:ring-1 focus:ring-slate-300 w-20 text-left {{ $statusClassSelect }}"
                                                                onchange="this.className='appearance-none rounded-full px-2.5 py-0.5 text-[10px] font-semibold border-0 cursor-pointer focus:ring-1 focus:ring-slate-300 w-20 text-left ' + (this.value==='on time' ? 'bg-green-100 text-green-600' : (this.value==='late' ? 'bg-yellow-100 text-yellow-600' : (this.value==='excused' ? 'bg-blue-100 text-blue-600' : 'bg-red-100 text-red-600')) )"
                                                                wire:change="updateAttendanceStatus({{ $attendanceId }}, {{ $userId }}, $event.target.value, '{{ addslashes($fullNameWithTitles) }}', '{{ $wd }}')"
                                                            >
                                                                <option value="on time" {{ $statusValue === 'on time' ? 'selected' : '' }}>On Time</option>
                                                                <option value="late" {{ $statusValue === 'late' ? 'selected' : '' }}>Late</option>
                                                                <option value="excused" {{ $statusValue === 'excused' ? 'selected' : '' }}>Excused</option>
                        ...
                        <div class="mt-6 pt-4 border-t border-slate-200">
                            ...
                            @if(!empty($monthlyAnalytics) && count($monthlyAnalytics) > 0)
                                @php
                                    ...
                                    // Aggregate monthly data by label to combine AM/PM entries
                                    $monthAgg = [];
                                    foreach ($monthlyAnalytics as $mData) {
                                        $label = $mData['label'] ?? '';
                                        $days = $mData['total_days'] ?? 1;
                                        if (!isset($monthAgg[$label])) {
                                            $monthAgg[$label] = [
                                                'on_time' => 0,
                                                'late' => 0,
                                                'absent' => 0,
                                        ...
                                        $monthAgg[$label]['on_time'] += $mData['on_time'] ?? 0;
                                        $monthAgg[$label]['late'] += $mData['late'] ?? 0;
                                        $monthAgg[$label]['absent'] += $mData['absent'] ?? 0;
                                        $monthAgg[$label]['excused'] += $mData['excused'] ?? 0;
                                        $monthAgg[$label]['days'] = max($monthAgg[$label]['days'], $days);
                                    }

                                    $ma = $monthlyAnalyticsCombined && count($monthlyAnalyticsCombined) ? $monthlyAnalyticsCombined : $monthlyAnalytics;
                                    foreach ($ma as $label => $vals) {
                                        $monthLabels[] = is_string($label) ? $label : ($vals['label'] ?? $label);
                                        $denom = max(1, ($vals['total_days'] ?? $vals['days'] ?? 1) * 2);
                                        $monthOnTime[] = (($vals['on_time'] ?? 0) / $denom);
                                        $monthLate[] = (($vals['late'] ?? 0) / $denom);
                                        $monthAbsent[] = (($vals['absent'] ?? 0) / $denom);
                                        $monthExcused[] = (($vals['excused'] ?? 0) / $denom);
                                    }
                                ...
                                <div wire:ignore
                                    wire:key="student-monthly-chart-{{ $monthlyAnalytics[0]['label'] ?? ($monthlyAnalyticsCombined[0]['label'] ?? '') }}"
                                    x-data="{
                                        monthlyChart: null,
    ...
    {{-- Confirmation Modal --}}
    @if($showConfirmModal)
        <div id="confirmModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" wire:click.self="cancelConfirmModal">
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 overflow-hidden">
                ...
                <div class="px-6 py-4 bg-white flex justify-end gap-3">
                    ...
                    <button 
                        id="confirmModalConfirmBtn-students"
                        type="button" 
...
<script>
    document.addEventListener('open-confirm-modal', function () {
        // Wait for the modal element to exist in the DOM, then nudge it visible and focus Confirm
        const tryShow = (attemptsLeft = 40) => {
            const modal = document.getElementById('confirmModal');
            const btn = document.getElementById('confirmModalConfirmBtn-students') || document.querySelector('[id^="confirmModalConfirmBtn-"]');
            if (!modal && !btn) {
                if (attemptsLeft > 0) return setTimeout(() => tryShow(attemptsLeft - 1), 100);
                return;
            }
            if (modal) {
