...
<div class="attendance-page" style="font-family: 'Inter', 'Segoe UI', system-ui, sans-serif;">
    <div class="attendance-wrapper">
            ...
            <div class="px-5 pb-4">
                <form wire:submit.prevent="applyFilters" class="flex items-end gap-3 rounded-lg border border-slate-200 bg-card p-3">
                    ...
                    <div class="flex flex-col gap-1 flex-1">
                        ...
                        <select wire:model.defer="positionFilter" class="w-full appearance-none rounded-md border border-slate-200 bg-card px-3 py-2 text-sm text-slate-600 focus:border-slate-400 focus:outline-none" style="background-image: url(&quot;data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e&quot;); background-position: right 0.5rem center; background-repeat: no-repeat; background-size: 1.5em 1.5em; padding-right: 2.5rem;">
        ...
        {{-- Confirmation Modal (always rendered; toggled visible via inline style) --}}
    <div id="confirmModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" wire:click.self="cancelConfirmModal" style="{{ $showConfirmModal ? '' : 'display:none' }}">
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 overflow-hidden">
            ...
            <div class="px-6 py-4 bg-white flex justify-end gap-3">
                ...
                <button 
                    id="confirmModalConfirmBtn-volunteers"
                    type="button" 
    ...
    @endif
    <script>
        // Use the same event the Students view listens for so behavior is identical
        document.addEventListener('open-confirm-modal', function () {
            const tryShow = (attemptsLeft = 40) => {
                const modal = document.getElementById('confirmModal');
                const btn = document.getElementById('confirmModalConfirmBtn-volunteers') || document.querySelector('[id^="confirmModalConfirmBtn-"]');
                if (!modal && !btn) {
