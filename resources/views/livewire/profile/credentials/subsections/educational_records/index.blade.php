<div class="profile-section">
    <div class="profile-section-header">
        <div class="flex items-center gap-3">
            <div class="inline-flex items-center gap-2">
                <flux:icon name="graduation-cap" class="w-5 h-5 text-gray-500 dark:text-gray-300" />
                ...
                <thead class="bg-gray-50 dark:bg-zinc-900"
                    ...
                    <tr>
                            <th class="px-2 py-2 text-center text-sm font-medium text-gray-500 dark:text-gray-300 w-12">
                                ...
                                <input type="checkbox" @click.stop wire:model="selectAll" wire:change="$refresh" wire:key="selectAllHeader" x-bind:checked="selectedLocal.length === {{ $records->count() }}" class="select-all form-checkbox mx-auto accent-blue-600 dark:accent-gray-300" aria-label="Select all on page" x-cloak />
                            </th>
                        @php $insertedYear = false; @endphp
                        @foreach($fields as $field)
                            @if(in_array($field['key'], ['year_started','year_graduated']))
                                @if(! $insertedYear)
                                    <th class="px-4 py-2 text-left text-sm font-medium text-gray-500 dark:text-gray-300">Year</th>
                                    @php $insertedYear = true; @endphp
                                @endif
                            @else
                                ...
                                @if(in_array($field['key'], ['year_started','year_graduated']))
                                    @if(! $insertedYear)
                                        <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">
                                            {{ $item->year_started ?? '—' }} — {{ $item->year_graduated ?? '—' }}
                                        </td>
                                        @php $insertedYear = true; @endphp
                                    @endif
                                @else
        ...
        <div class="grid grid-cols-1 {{ $records->count() > 1 ? 'md:grid-cols-2' : 'md:grid-cols-1' }} gap-4" x-data="{ selectedLocal: @entangle('selected') }">
            @foreach($records as $item)
                ...
                <div wire:key="card-row-{{ $rowKeyStr }}" class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded p-4 relative"
                    ...
                    <div class="space-y-1">
                        <div class="text-sm font-semibold text-gray-800 dark:text-gray-100">{{ optional($item->degreeProgram)->name ?? '—' }}</div>
                        <div class="text-sm text-gray-600 dark:text-gray-300">{{ optional($item->university)->name ?? '—' }}</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $item->year_started ?? '—' }} — {{ $item->year_graduated ?? '—' }}</div>
                    </div>
    ...
    <livewire:profile.credentials.subsections.educational-records.educational-records-form-modal instance-key="App\\Models\\EducationalRecord" :key="'educational-records-form-'.$user->id" />

    <livewire:profile.credentials.subsections.educational-records.educational-records-details-modal :key="'educational-records-details-'.$user->id" />

    <livewire:profile.credentials.subsections.educational-records.delete-modal :key="'educational-records-delete-'.$user->id" />
    <livewire:profile.credentials.subsections.educational-records.restore-modal :key="'educational-records-restore-'.$user->id" />
    <livewire:profile.credentials.subsections.educational-records.force-delete-modal :key="'educational-records-force-delete-'.$user->id" />
    
    {{-- Archive modal mount (scoped to this subsection) --}}
    <livewire:profile.credentials.subsections.educational-records.educational-records-archive :key="'educational-records-archive-'.$user->id" />
</div>
