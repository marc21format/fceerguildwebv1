    ...
    @if($isOpen)
        <div class="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
            <flux:modal name="highschool-records-form" flyout class="w-11/12 max-w-2xl" wire:model="isOpen" @close="$set('isOpen', false)">
                <div class="space-y-6">
                    <div>
                        <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white">{{ $itemId ? 'Edit' : 'Create' }} Highschool Record</h2>
                        ...
                        @foreach($fields as $f)
                            <div>
                                ...
                                @if(($f['type'] ?? '') === 'select')
                                    ...
                                    @endphp
                                    @include('livewire.components.searchable-select', ['name' => $f['key'], 'options' => $optsOut, 'selected' => $sel ?? '', 'placeholder' => 'Select'])
