<div class="profile-section">
    <div class="profile-section-header">
        <div class="flex items-center gap-3">
            <div class="inline-flex items-center gap-2">
                <flux:icon name="school" class="w-5 h-5 text-gray-700 dark:text-gray-300" />
                ...
                <thead class="bg-gray-50 dark:bg-zinc-900"
                    ...
                    <tr>
                        ...
                        @foreach($fields as $field)
                            @if(in_array($field['key'], ['year_started','year_ended']))
                                @if(! $insertedYear)
        ...
        <div class="grid grid-cols-1 {{ $records->count() > 1 ? 'md:grid-cols-2' : 'md:grid-cols-1' }} gap-4" x-data="{ selectedLocal: @entangle('selected') }">
            @foreach($records as $item)
                ...
                <div wire:key="card-row-{{ $rowKeyStr }}" class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded p-4 relative"
                    ...
                    <div class="space-y-1">
                        <div class="text-sm font-semibold text-gray-800 dark:text-gray-100">{{ optional($item->highschool)->name ?? '—' }}</div>
                        @php
                            $lvl = $item->level ?? null;
                            if ($lvl) {
                        ...
                        @endphp
                        <div class="text-sm text-gray-600 dark:text-gray-300">{{ $displayLevel }}</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $item->year_started ?? '—' }} — {{ $item->year_ended ?? '—' }}</div>
                    </div>
    ...
    <livewire:profile.credentials.subsections.highschool-records.highschool-records-form-modal instance-key="App\\Models\\HighschoolRecord" :key="'highschool-records-form-'.$user->id" />

    <livewire:profile.credentials.subsections.highschool-records.details-modal :key="'highschool-records-details-'.$user->id" />
    <livewire:profile.credentials.subsections.highschool-records.highschool-records-archive :key="'highschool-records-archive-'.$user->id" />

    <livewire:profile.credentials.subsections.highschool-records.delete-modal :key="'highschool-records-delete-'.$user->id" />
    <livewire:profile.credentials.subsections.highschool-records.restore-modal :key="'highschool-records-restore-'.$user->id" />
    <livewire:profile.credentials.subsections.highschool-records.force-delete-modal :key="'highschool-records-force-delete-'.$user->id" />
</div>
