<div class="profile-section">
    <div class="profile-section-header">
        <div class="flex items-center gap-3">
            <div class="inline-flex items-center gap-2">
                <flux:icon name="notebook-pen" class="w-5 h-5 text-gray-700 dark:text-gray-300" />
                ...
                <thead class="bg-gray-50 dark:bg-zinc-900"
                    ...
                    <tr>
                        <th class="px-2 py-2 text-center text-sm font-medium text-gray-500 dark:text-gray-300 w-12">
                            ...
                            <input type="checkbox" @click.stop wire:model="selectAll" wire:change="$refresh" wire:key="selectAllHeaderSubject" x-bind:checked="selectedLocal.length === {{ $items->count() }}" class="select-all form-checkbox mx-auto accent-blue-600 dark:accent-gray-300" aria-label="Select all on page" x-cloak />
                        </th>
                        ...
                        <tr
                            ...
                            <td class="px-2 py-2 text-sm text-gray-700 dark:text-gray-200">
                                <div class="flex items-center justify-center">
                                    ...
                                    <input type="checkbox" @click.stop wire:model="selected" wire:change="$refresh" wire:key="selected-subject.{{ $rowKeyStr }}" value="{{ $rowKeyStr }}" class="row-checkbox form-checkbox mx-auto accent-blue-600 dark:accent-gray-300" x-cloak />
                                </div>
                            </td>

                            <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">{{ optional($item->subject)->name ?? $item->highschool_subject_id }}</td>
                            <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">{{ ucfirst($item->grade) }}</td>
        ...
        <div class="grid grid-cols-1 {{ $items->count() > 1 ? 'md:grid-cols-2' : 'md:grid-cols-1' }} gap-4" x-data="{ selectedLocal: @entangle('selected') }">
            @foreach($items as $item)
                @php
                    ...
                    $rowKeyStr = (string) $rowKey;
                @endphp
                <div wire:key="card-row-subject-{{ $rowKeyStr }}" class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded p-4 relative"
                        x-data="{
                        id: 'card-menu-subject-{{ $rowKeyStr }}',
                        top: 0,
                        left: 0,
                        init() { if (!Alpine.store('menu')) Alpine.store('menu', { openId: null }); },
                        get open() { return Alpine.store('menu').openId === this.id },
                    ...
                    <div class="absolute inset-y-2 right-2 flex flex-col justify-between items-center">
                        ...
                        <input x-cloak type="checkbox" @click.stop wire:model="selected" wire:change="$refresh" wire:key="selected-subject.{{ $rowKeyStr }}" value="{{ $rowKeyStr }}" aria-label="Select record"
                            class="row-checkbox form-checkbox mx-auto accent-blue-600 dark:accent-gray-300 h-4 w-4 transition-opacity duration-150"
                            :class="($store.rowHover.hovered === '{{ $rowKeyStr }}' || selectedLocal.includes('{{ $rowKeyStr }}')) ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'" />
                    </div>

                    <div class="space-y-1">
                        <div class="text-sm font-semibold text-gray-800 dark:text-gray-100">{{ optional($item->subject)->name ?? '—' }}</div>
                        <div class="text-sm text-gray-600 dark:text-gray-300">{{ ucfirst($item->grade) }}</div>
                    </div>
    ...
    <livewire:profile.credentials.subsections.highschool-subject-records.highschool-subject-records-form-modal instance-key="App\\Models\\HighschoolSubjectRecord" :key="'highschool-subject-records-form-'.$user->id" />

    <livewire:profile.credentials.subsections.highschool-subject-records.highschool-subject-records-details-modal :key="'highschool-subject-records-details-'.$user->id" />
    <livewire:profile.credentials.subsections.highschool-subject-records.highschool-subject-records-archive :key="'highschool-subject-records-archive-'.$user->id" />

    <livewire:profile.credentials.subsections.highschool-subject-records.highschool-subject-delete-modal :key="'highschool-subjects-delete-'.$user->id" />
    <livewire:profile.credentials.subsections.highschool-subject-records.highschool-subject-restore-modal :key="'highschool-subjects-restore-'.$user->id" />
    <livewire:profile.credentials.subsections.highschool-subject-records.highschool-subject-force-delete-modal :key="'highschool-subjects-force-delete-'.$user->id" />
</div>
