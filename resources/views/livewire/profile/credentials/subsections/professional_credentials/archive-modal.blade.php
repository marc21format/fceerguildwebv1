<div>
    @if(isset($renderTrigger) && $renderTrigger)
        <div class="ml-3">
            <flux:modal.trigger name="professional-archive">
    ...
    @if($open)
        <div class="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-end justify-center z-50">
            <flux:modal name="professional-archive" flyout position="bottom" class="md:w-lg" wire:model="open" @close="$set('open', false)" role="dialog" aria-modal="true">
                <div class="space-y-4">
                    <flux:heading size="lg">Archive</flux:heading>
                    <flux:subheading>Manage soft-deleted professional credentials. Restore or permanently delete items.</flux:subheading>
                                    ...
                                    <tbody class="divide-y">
                                        @forelse($items as $item)
                                            ...
                                            <tr class="group bg-white dark:bg-zinc-800" wire:key="archive-row-{{ $rowKeyStr }}" @click.prevent="if ($event.target.closest('button, input, a')) return; $wire.toggleRow('{{ $rowKeyStr }}')" x-on:mouseenter="$store.rowHover.hovered = '{{ $rowKeyStr }}'" x-on:mouseleave="$store.rowHover.hovered = null">
                                                ...
                                                <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">{{ optional($item->fieldOfWork)->name ?? '—' }}</td>
                                                <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">{{ optional($item->prefix)->name ? (optional($item->prefix)->name . (optional($item->prefix)->abbreviation ? ' (' . optional($item->prefix)->abbreviation . ')' : '')) : (optional($item->suffix)->title ? (optional($item->suffix)->title . (optional($item->suffix)->abbreviation ? ' (' . optional($item->suffix)->abbreviation . ')' : '')) : '—') }}</td>
                                                <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">{{ $item->issued_on ?? '' }}</td>
