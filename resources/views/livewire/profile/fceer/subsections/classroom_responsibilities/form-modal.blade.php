...
<div>
    @if($isOpen)
        <div class="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
            <flux:modal name="classroom-responsibilities-form" flyout class="w-11/12 max-w-2xl" wire:model="isOpen" @close="$set('isOpen', false)">
                <div class="space-y-6">
                    <div>
                        <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $itemId ? 'Edit' : 'Create' }} Classroom Responsibility</h2>
                    ...
                    @if($hasDuplicate)
                        <div class="mt-2 p-3 rounded-md bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 text-sm text-yellow-800 dark:text-yellow-200">
                            <strong>Notice:</strong> This classroom responsibility already exists for user: <strong>{{ $duplicate['user_name'] ?? 'Unknown' }}</strong>
