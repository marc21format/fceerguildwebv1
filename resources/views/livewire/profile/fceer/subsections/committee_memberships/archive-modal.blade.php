<div>
    @if($renderTrigger ?? false)
        <div class="ml-3">
            <flux:modal.trigger name="committee-memberships-archive">
    ...
    @if($open ?? false)
        <div class="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-end justify-center z-50">
            <flux:modal name="committee-memberships-archive" flyout position="bottom" class="md:w-lg" wire:model="open" @close="$set('open', false)" role="dialog" aria-modal="true">
                <div class="space-y-4">
                                    ...
                                    <tbody class="divide-y">
                                        @forelse($items as $item)
                                            ...
                                            <tr class="group bg-white dark:bg-zinc-800" wire:key="archive-row-{{ $rowKeyStr }}" @click.prevent="if ($event.target.closest('button, input, a')) return; $wire.toggleRow('{{ $rowKeyStr }}')" x-on:mouseenter="$store.rowHover.hovered = '{{ $rowKeyStr }}'" x-on:mouseleave="$store.rowHover.hovered = null">
                                                ...
                                                <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">{{ optional($item->committee)->name ?? '—' }}</td>
                                                <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">{{ optional($item->committeePosition)->name ?? '—' }}</td>
