<div class="profile-section">
                ...
                <tbody class="divide-y">
                    @foreach($records as $item)
                        ...
                        <tr
                            ...
                            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">{{ optional($item->committee)->name ?? ($item->committee_id ? $item->committee_id : '—') }}</td>
                            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">{{ optional($item->committeePosition)->name ?? ($item->committee_position_id ? $item->committee_position_id : '—') }}</td>
        ...
        <div class="grid grid-cols-1 {{ $records->count() > 1 ? 'md:grid-cols-2' : 'md:grid-cols-1' }} gap-4" x-data="{ selectedLocal: @entangle('selected') }">
            @foreach($records as $item)
                ...
                <div wire:key="card-row-{{ $rowKeyStr }}" class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded p-4 relative"
                    ...
                    <div class="space-y-1">
                        <div class="text-md text-gray-900 dark:text-white">{{ optional($item->committee)->name ?? '—' }}</div>
                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ optional($item->committeePosition)->name ?? '—' }}</div>
                    </div>
    ...
    <livewire:profile.fceer.subsections.committee-memberships.committee-memberships-form-modal instance-key="App\\Models\\CommitteeMembership" :key="'committee-memberships-form-'.$user->id" />
    <livewire:profile.fceer.subsections.committee-memberships.committee-memberships-confirm-modal :key="'committee-memberships-confirm-'.$user->id" />
    <livewire:profile.fceer.subsections.committee-memberships.committee-memberships-archive :key="'committee-memberships-archive-'.$user->id" />
    <livewire:profile.fceer.subsections.committee-memberships.committee-memberships-delete-modal :key="'committee-memberships-delete-'.$user->id" />
    <livewire:profile.fceer.subsections.committee-memberships.committee-memberships-details-modal :key="'committee-memberships-details-'.$user->id" />
    <livewire:profile.fceer.subsections.committee-memberships.committee-memberships-force-delete-modal :key="'committee-memberships-force-delete-'.$user->id" />
    <livewire:profile.fceer.subsections.committee-memberships.committee-memberships-restore-modal :key="'committee-memberships-restore-'.$user->id" />
</div>
