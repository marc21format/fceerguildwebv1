<div class="profile-section">
    <div class="profile-section-header">
        <div class="flex items-center gap-3">
            <div class="inline-flex items-center gap-2">
                <flux:icon name="book-open" class="w-5 h-5 text-gray-700 dark:text-gray-300" />
                ...
                <tbody class="divide-y">
                    @foreach($records as $item)
                        ...
                        <tr
                            ...
                            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">{{ optional($item->volunteerSubject)->name ?? ($item->volunteer_subject_id ? $item->volunteer_subject_id : '—') }}</td>
                            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">{{ $item->subject_proficiency ?? '—' }}</td>
        ...
        <div class="grid grid-cols-1 {{ $records->count() > 1 ? 'md:grid-cols-2' : 'md:grid-cols-1' }} gap-4" x-data="{ selectedLocal: @entangle('selected') }">
            @foreach($records as $item)
                ...
                <div wire:key="card-row-{{ $rowKeyStr }}" class="bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded p-4 relative"
                    ...
                    <div class="space-y-1">
                        <div class="text-md text-gray-900 dark:text-white">{{ optional($item->volunteerSubject)->name ?? '—' }}</div>
                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ $item->subject_proficiency ?? '—' }}</div>
                    </div>
    ...
    <livewire:profile.fceer.subsections.subject-teachers.subject-teachers-form-modal instance-key="App\\Models\\SubjectTeacher" :key="'subject-teachers-form-'.$user->id" />
    <livewire:profile.fceer.subsections.subject-teachers.subject-teachers-confirm-modal :key="'subject-teachers-confirm-'.$user->id" />
    <livewire:profile.fceer.subsections.subject-teachers.subject-teachers-archive-modal :key="'subject-teachers-archive-'.$user->id" />
    <livewire:profile.fceer.subsections.subject-teachers.subject-teacher-delete-modal :key="'subject-teacher-delete-'.$user->id" />
    <livewire:profile.fceer.subsections.subject-teachers.subject-teachers-details-modal :key="'subject-teachers-details-'.$user->id" />
    <livewire:profile.fceer.subsections.subject-teachers.subject-teacher-force-delete-modal :key="'subject-teacher-force-delete-'.$user->id" />
    <livewire:profile.fceer.subsections.subject-teachers.subject-teacher-restore-modal :key="'subject-teacher-restore-'.$user->id" />
</div>
