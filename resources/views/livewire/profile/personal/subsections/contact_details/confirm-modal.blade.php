<div>
    @if($open ?? false)
        <div class="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
            <flux:modal name="contact-details-confirm" class="w-11/12 max-w-2xl" wire:model="open" @close="$set('open', false)">
                <div class="space-y-6">
                    <div>
                        <flux:heading size="lg">Confirm Contact Details Changes</flux:heading>
                    </div>
                                    ...
                                    @foreach($changes as $key => $c)
                                        <tr class="bg-white dark:bg-zinc-800">
                                            ...
                                            <td class="px-3 py-2 text-sm text-gray-600 dark:text-gray-300">
                                                {{ strlen((string)($c['old'] ?? '')) ? (is_array($c['old']) ? json_encode($c['old']) : $c['old']) : '—' }}
                                            </td>
                                            <td class="px-3 py-2 text-sm text-gray-700 dark:text-gray-200">
                                                {{ strlen((string)($c['new'] ?? '')) ? (is_array($c['new']) ? json_encode($c['new']) : $c['new']) : '—' }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
