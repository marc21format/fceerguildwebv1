<div>
    @if($open ?? false)
        <div class="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-end justify-center z-50" @click.self="$set('open', false)">
            <flux:modal name="roster-archive" flyout position="bottom" class="md:w-lg" wire:model="open" @close="$set('open', false)" role="dialog" aria-modal="true">
                <div class="space-y-4">
                    <flux:heading size="lg">Archive - {{ ucfirst($type) }}</flux:heading>
                    <flux:subheading>Manage soft-deleted {{ $type }}. Restore or permanently delete users.</flux:subheading>

                    <div class="space-y-3" x-data="{ selectedLocal: @entangle('selected') }">
                        @php
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                ...
                                @if(($selectedCount ?? 0) > 0)
                                    <div class="text-sm text-indigo-600 dark:text-gray-100">{{ $selectedCount }} selected @if(count($selectedRows))({{ implode(',', $selectedRows) }})@endif</div>
                                @endif
                            </div>
                            <div class="flex items-center gap-2">
                                <flux:button size="sm" wire:click.prevent="restoreSelected" x-bind:disabled="selectedLocal.length === 0" icon:leading="arrow-uturn-left">Restore</flux:button>
                                @can('forceDeleteRosterUser')
                                <flux:button size="sm" tone="danger" wire:click.prevent="forceDeleteSelected" x-bind:disabled="selectedLocal.length === 0" icon:leading="trash">Permanently Delete</flux:button>
                                @endcan
                            </div>
                        </div>
                                ...
                                <table class="min-w-full divide-y">
                                    <thead class="bg-gray-50 dark:bg-zinc-900" x-on:mouseenter="$store.rowHover.hovered = 'archive'" x-on:mouseleave="$store.rowHover.hovered = null">
                                        <tr>
                                            ...
                                            <th class="px-4 py-2 text-left text-sm font-medium text-gray-500 dark:text-gray-300">{{ $type === 'students' ? 'Student' : 'Volunteer' }} #</th>
                                    ...
                                    <tbody class="divide-y">
                                        @forelse($items as $item)
                                            ...
                                            <tr class="group bg-white dark:bg-zinc-800" wire:key="archive-row-{{ $rowKeyStr }}" @click.prevent="if ($event.target.closest('button, input, a')) return; $wire.toggleRow('{{ $rowKeyStr }}')" x-on:mouseenter="$store.rowHover.hovered = '{{ $rowKeyStr }}'" x-on:mouseleave="$store.rowHover.hovered = null">
                                                ...
                                                <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">{{ $item->fceerProfile?->{$numberField} ?? '—' }}</td>
                                                <td class="px-4 py-2 text-sm text-gray-700 dark:text-gray-200">{{ $item->deletedBy?->name ?? '—' }}</td>
                                                    ...
                                                    <div x-data="{ id: 'archive-menu-{{ $rowKeyStr }}', top: 0, left: 0, init() { if (!Alpine.store('menu')) Alpine.store('menu', { openId: null }); }, get open() { return Alpine.store('menu').openId === this.id }, disableScroll() { const container = document.querySelector('.reference-archive-modal-content'); if (container) container.style.overflow = 'hidden'; document.body.style.overflow = 'hidden'; }, restoreScroll() { const container = document.querySelector('.reference-archive-modal-content'); if (container) container.style.overflow = ''; document.body.style.overflow = ''; }, openMenu(ref) { const rect = ref.getBoundingClientRect(); const menuWidth = 220; const minLeft = 8; const desiredLeft = rect.right - menuWidth; const maxLeft = Math.max(minLeft, window.innerWidth - menuWidth - 8); this.top = Math.round(rect.bottom); this.left = Math.min(Math.max(minLeft, Math.round(desiredLeft)), maxLeft); const newId = (Alpine.store('menu').openId === this.id ? null : this.id); Alpine.store('menu').openId = newId; if (newId === this.id) this.disableScroll(); else this.restoreScroll(); }, close() { if (Alpine.store('menu').openId === this.id) { Alpine.store('menu').openId = null; this.restoreScroll(); } } }">
                                                        ...
                                                        <div x-show="open" x-cloak @keydown.escape.window="close()" @click.away="close()" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100"
                                                            ...
                                                            <div class="flex flex-col divide-y">
                                                                <button type="button" @click.stop="close(); $dispatch('openArchiveRestoreModal', { userId: {{ $item->id }} })" class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-zinc-700">
                                                                    <flux:icon name="arrow-uturn-left" class="w-4 h-4" />
                                                                    Restore
                                                                </button>
                                                                @can('forceDeleteRosterUser')
                                                                <button type="button" @click.stop="close(); $dispatch('openRosterForceDeleteModal', { userId: {{ $item->id }} })" class="flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-gray-50 dark:hover:bg-zinc-700">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="7" class="px-4 py-6 text-sm text-gray-500 text-center">No archived {{ $type }} found.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                        ...
                        @else
                            <div class="px-4 py-6 text-sm text-gray-500 text-center">No archived {{ $type }} available.</div>
                        @endif
                    </div>
                </div>
            </flux:modal>
    ...
    {{-- Include the modal component globally --}}
    @once
