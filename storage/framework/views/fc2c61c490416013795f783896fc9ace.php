...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'e60dd9d2c3a62d619c9acb38f20d5aa5::icon.index','data' => ['name' => 'academic-cap','class' => 'w-5 h-5 text-gray-700 dark:text-gray-300']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['name' => 'academic-cap','class' => 'w-5 h-5 text-gray-700 dark:text-gray-300']); ?>
...
<?php endif; ?>
        ...
        <div class="inline-flex items-center space-x-2">
            ...
            ?>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(count($selected) && $canManage): ?>
                <div class="text-sm text-indigo-600 dark:text-gray-300 mr-2"><?php echo e(count($selected)); ?> record/s selected <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($selectedRowNumbers)): ?>: <span class="font-mono text-sm"><?php echo e(implode(',', $selectedRowNumbers)); ?></span><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?></div>
...
<?php endif; ?>
    ...
    <?php if (! $__env->hasRenderedOnce('6d61af05-bb5f-40f6-bc7d-289d4d066fdf')): $__env->markAsRenderedOnce('6d61af05-bb5f-40f6-bc7d-289d4d066fdf'); ?>
                ...
                <thead class="bg-gray-50 dark:bg-zinc-900"
                    ...
                    <tr>
                            <th class="px-2 py-2 text-center text-sm font-medium text-gray-500 dark:text-gray-300 w-12">
                                ...
                                <input x-ref="selectAll" type="checkbox" @click.stop wire:model="selectAll" wire:change="$refresh" wire:key="selectAllHeader" x-bind:checked="selectedLocal.length === <?php echo e($records->count()); ?>" x-effect="$refs.selectAll.indeterminate = (selectedLocal.length > 0 && selectedLocal.length < <?php echo e($records->count()); ?>)" class="select-all form-checkbox mx-auto accent-blue-600 dark:accent-gray-300" aria-label="Select all on page" x-cloak />
                            </th>
                ...
                <tbody class="divide-y">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $records; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <tr
                            ...
                            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400"><?php echo e(optional($item->classroom)->name ?? ($item->classroom_id ? $item->classroom_id : '—')); ?></td>
                            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400"><?php echo e(optional($item->classroomPosition)->name ?? ($item->classroom_position_id ? $item->classroom_position_id : '—')); ?></td>
                            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400"><?php echo e($item->note ?? '—'); ?></td>
...
<?php endif; ?>
                    ...
                    <div class="space-y-1">
                        <div class="text-md text-gray-900 dark:text-white"><?php echo e(optional($item->classroom)->name ?? '—'); ?></div>
                        <div class="text-sm text-gray-500 dark:text-gray-400"><?php echo e(optional($item->classroomPosition)->name ?? '—'); ?></div>
                    </div>

                    <div x-data="{ id: 'classroom-responsibility-card-menu-<?php echo e($rowKeyStr); ?>', top: 0, left: 0, init() { if (!Alpine.store('menu')) Alpine.store('menu', { openId: null }); }, get open() { return Alpine.store('menu').openId === this.id }, disableScroll() { const container = document.querySelector('.reference-table-container'); if (container) container.style.overflow = 'hidden'; document.body.style.overflow = 'hidden'; }, restoreScroll() { const container = document.querySelector('.reference-table-container'); if (container) container.style.overflow = ''; document.body.style.overflow = ''; }, openMenu(ref) { const rect = ref.getBoundingClientRect(); const menuWidth = 160; const minLeft = 8; const desiredLeft = rect.right - menuWidth; const maxLeft = Math.max(minLeft, window.innerWidth - menuWidth - 8); this.top = Math.round(rect.bottom); this.left = Math.min(Math.max(minLeft, Math.round(desiredLeft)), maxLeft); const newId = (Alpine.store('menu').openId === this.id ? null : this.id); Alpine.store('menu').openId = newId; if (newId === this.id) this.disableScroll(); else this.restoreScroll(); }, close() { if (Alpine.store('menu').openId === this.id) { Alpine.store('menu').openId = null; this.restoreScroll(); } } }">
                        <div x-show="open" x-cloak @keydown.escape.window="close()" @click.away="close()" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100"
                            :style="`position: fixed; top: ${top}px; left: ${left}px;`"
...
[$__name, $__params] = $__split('profile.fceer.subsections.classroom-responsibilities.classroom-responsibilities-form-modal', ['instanceKey' => 'App\\Models\\ClassroomResponsibility']);

$key = 'classroom-responsibilities-form-'.$user->id;

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-3193436141-0', 'classroom-responsibilities-form-'.$user->id);
...
[$__name, $__params] = $__split('profile.fceer.subsections.classroom-responsibilities.classroom-responsibilities-confirm-modal', []);

$key = 'classroom-responsibilities-confirm-'.$user->id;

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-3193436141-1', 'classroom-responsibilities-confirm-'.$user->id);
...
[$__name, $__params] = $__split('profile.fceer.subsections.classroom-responsibilities.classroom-responsibilities-archive', []);

$key = 'classroom-responsibilities-archive-'.$user->id;

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-3193436141-2', 'classroom-responsibilities-archive-'.$user->id);
...
[$__name, $__params] = $__split('profile.fceer.subsections.classroom-responsibilities.classroom-responsibilities-delete-modal', []);

$key = 'classroom-responsibilities-delete-'.$user->id;

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-3193436141-3', 'classroom-responsibilities-delete-'.$user->id);
...
[$__name, $__params] = $__split('profile.fceer.subsections.classroom-responsibilities.classroom-responsibilities-details-modal', []);

$key = 'classroom-responsibilities-details-'.$user->id;

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-3193436141-4', 'classroom-responsibilities-details-'.$user->id);
...
[$__name, $__params] = $__split('profile.fceer.subsections.classroom-responsibilities.classroom-responsibilities-force-delete-modal', []);

$key = 'classroom-responsibilities-force-delete-'.$user->id;

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-3193436141-5', 'classroom-responsibilities-force-delete-'.$user->id);
...
[$__name, $__params] = $__split('profile.fceer.subsections.classroom-responsibilities.classroom-responsibilities-restore-modal', []);

$key = 'classroom-responsibilities-restore-'.$user->id;

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-3193436141-6', 'classroom-responsibilities-restore-'.$user->id);
...
<?php /**PATH C:\Users\Marc Ian C. Young\laravel\fceerguildweb\resources\views/livewire/profile/fceer/subsections/classroom_responsibilities/index.blade.php ENDPATH**/ ?>